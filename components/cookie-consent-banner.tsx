...
export function CookieConsentBanner() {
  ...
  return <div className="fixed inset-x-0 bottom-0 z-[200] flex justify-center p-3 sm:bottom-4 sm:p-4">
    <div className="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white p-5 shadow-lift sm:p-6">
      <div className="flex items-start gap-3">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-teal-50 text-teal-600"><Cookie size={19} /></span>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-extrabold text-ink">We use cookies</p>
        ...
        <button type="button" onClick={reject} aria-label="Dismiss and keep only essential cookies" className="shrink-0 cursor-pointer rounded-full p-1.5 text-slate-300 transition hover:bg-slate-100 hover:text-ink"><X size={16} /></button>
      </div>

      {managing && <div className="mt-4 divide-y divide-slate-100 rounded-2xl border border-slate-100 bg-[#f9fcfc] px-4">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-end gap-2">
        {!managing && <button type="button" onClick={() => setManaging(true)} className="mr-auto cursor-pointer text-xs font-bold text-slate-500 transition hover:text-ink">Manage preferences</button>}
        {managing
          ...
          : <>
            <button type="button" onClick={reject} className="cursor-pointer rounded-full border border-slate-200 px-5 py-2.5 text-xs font-bold text-ink transition hover:border-teal-200 hover:bg-[#f9fcfc]">Reject non-essential</button>
