...
export function Navigation() {
  ...
  return <><LanguageBar className="fixed inset-x-0 top-0 z-[60]" /><header className={`fixed inset-x-0 top-9 z-50 transition-all duration-300 ${scrolled ? "border-b border-slate-200/70 dark:border-white/10 bg-white/85 dark:bg-[#0d1917]/85 py-3 shadow-sm backdrop-blur-xl" : "py-5"}`}><nav className="container-page flex items-center justify-between"><Logo className="h-10 w-auto" /><div className="hidden items-center gap-7 md:flex">{links.map(x => <a key={x} href={linkHref(x)} className="cursor-pointer text-sm font-bold text-slate-500 transition hover:text-heading">{x}</a>)}<a href="/login" className="cursor-pointer text-sm font-bold text-slate-600 hover:text-heading">Log in</a><PrimaryButton className="px-4 py-2.5">Get started</PrimaryButton></div><button type="button" className="grid h-11 w-11 cursor-pointer place-items-center rounded-full border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d1917] md:hidden" aria-label="Toggle menu" onClick={() => setOpen(!open)}>{open ? <X size={19} /> : <Menu size={19} />}</button></nav>{open && <div className="container-page mt-3 md:hidden"><div className="rounded-2xl border bg-white dark:bg-[#0d1917] p-3 shadow-lift">{links.map(x => <a onClick={() => setOpen(false)} key={x} href={linkHref(x)} className="block cursor-pointer rounded-xl px-4 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 dark:bg-white/5">{x}</a>)}<a href="/login" onClick={() => setOpen(false)} className="block cursor-pointer rounded-xl px-4 py-3 text-sm font-bold text-slate-600 hover:bg-slate-50 dark:bg-white/5">Log in</a><a href="/signup" onClick={() => setOpen(false)} className="mt-1 block cursor-pointer rounded-xl bg-accent-500 px-4 py-3 text-center text-sm font-bold text-white">Start studying free</a></div></div>}</header></>;
}
