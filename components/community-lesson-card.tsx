...
export function CommunityLessonCard({ lesson, onSave, onUnsave }: { lesson: CommunityLesson; onSave: (id: string) => void; onUnsave: (id: string) => void }) {
  ...
  return <div className="flex flex-col rounded-3xl border border-slate-200 bg-[#fdfefe] p-5 shadow-soft transition hover:-translate-y-0.5 hover:shadow-lift">
    ...
    <Link href={`/dashboard/library/community/${lesson.id}`} className="mt-1.5 cursor-pointer text-base font-extrabold tracking-tight text-ink hover:text-teal-700">{lesson.title}</Link>
    <p className="mt-2 flex-1 text-xs leading-relaxed text-slate-500">{lesson.description}</p>
    <div className="mt-3 flex items-center gap-1.5 text-[11px] font-bold text-slate-500"><span className="grid h-5 w-5 place-items-center rounded-full bg-slate-100 text-slate-500"><User size={10} /></span>Created by {lesson.creatorName}</div>
    ...
    <div className="mt-4 flex gap-2">
      <Link href={`/dashboard/library/community/${lesson.id}`} className="flex-1 cursor-pointer rounded-full border border-slate-200 px-4 py-2.5 text-center text-xs font-bold text-ink transition hover:border-teal-200 hover:bg-white">Preview</Link>
      {saved
        ? <button type="button" onClick={() => onUnsave(lesson.id)} className="flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-full border border-teal-500 bg-teal-50 px-4 py-2.5 text-xs font-bold text-teal-700"><Check size={13} />In My Library</button>
