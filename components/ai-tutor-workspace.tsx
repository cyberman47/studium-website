...
export function AiTutorWorkspace({ context }: { context: TutorContext }) {
  ...
  return <div className="flex h-full flex-col overflow-hidden bg-white">
    ...
    <div className="flex shrink-0 items-center gap-3 border-b border-slate-100 px-5 py-3">
      <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-teal-50 text-teal-600"><Bot size={16} /></span>
      <p className="shrink-0 text-sm font-extrabold text-ink">Studium AI</p>
      ...
      <div className="ml-auto flex shrink-0 items-center gap-2">
        ...
        <div className="flex items-center gap-0.5 rounded-lg border border-slate-200 bg-slate-50 p-0.5">
          {tabDefs.map(t => {
            ...
            return <button
              ...
              className={`flex cursor-pointer items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-bold transition ${active ? "bg-white text-teal-700 shadow-soft" : "text-slate-500 hover:text-ink"}`}
        ...
        <div className="relative">
          <button
            ...
            className="flex cursor-pointer items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-xs font-bold text-ink transition hover:border-teal-200"
            ...
            <div className="absolute right-0 z-20 mt-1.5 w-40 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-lg">
              {(Object.keys(modeLabels) as TutorMode[]).map(m => {
                ...
                return <button
                  ...
                  className={`flex w-full cursor-pointer items-center gap-2 px-3 py-2 text-left text-xs font-bold transition ${active ? "bg-teal-50 text-teal-700" : "text-slate-600 hover:bg-[#f9fcfc]"}`}
    ...
    {tab === "chat" && <>
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 py-8 sm:px-10">
        <div className="mx-auto flex max-w-3xl flex-col gap-5">
          {messages.length === 0 && <div className="flex flex-col items-center gap-4 py-16 text-center">
            <span className="grid h-12 w-12 place-items-center rounded-2xl bg-teal-50 text-teal-600"><Sparkles size={22} /></span>
            <div>
              <p className="text-base font-extrabold text-ink">Ask anything about {context.lessonTitle.toLowerCase()}.</p>
              <p className="mt-1 text-sm text-slate-400">Pick a mode above, then start typing—or try a prompt below.</p>
            </div>
            <div className="flex max-w-xl flex-wrap justify-center gap-2">
              {quickActions.map(qa => <button key={qa.label} type="button" onClick={() => handleSend(qa.build(context))} className="cursor-pointer rounded-full border border-slate-200 px-3.5 py-1.5 text-xs font-bold text-slate-600 transition hover:border-teal-200 hover:bg-teal-50 hover:text-teal-700">{qa.label}</button>)}
            </div>
          </div>}
          ...
          {messages.map(m => {
            ...
            const bubbleClass = m.error
              ...
              : "border border-slate-100 bg-[#fcfdfd] text-slate-600 shadow-soft";
      ...
      {/* Input bar — anchored to the bottom of the workspace */}
      <div className="shrink-0 border-t border-slate-100 bg-white px-5 py-4 sm:px-10">
        <div className="mx-auto max-w-3xl">
          <form onSubmit={e => { e.preventDefault(); handleSend(input); }} className="flex items-end gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-[0_8px_24px_-12px_rgba(15,23,42,0.12)] transition focus-within:border-teal-300">
            <input
              ...
              className="w-full bg-transparent px-2.5 py-2 text-sm text-ink outline-none placeholder:text-slate-400 disabled:cursor-not-allowed"
            />
