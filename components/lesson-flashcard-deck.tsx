...
export function LessonFlashcardDeck({
  ...
  return <div className="rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-[#0d1917] p-6 shadow-soft sm:p-7">
    ...
    <div className="flex items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-slate-100 dark:bg-white/10 px-2.5 py-1 text-[11px] font-extrabold tabular-nums text-slate-600">{cardIndex + 1} / {cards.length}</span>
        ...
        <button type="button" onClick={onFullscreen} title="Fullscreen" className="grid h-7 w-7 cursor-pointer place-items-center rounded-lg text-slate-400 transition hover:bg-slate-100 dark:bg-white/10 hover:text-heading"><Maximize size={15} /></button>
      </div>
    </div>
    ...
    <div className="mt-5 flex items-center justify-between gap-2">
      <button type="button" onClick={goPrev} disabled={atStart} title="Previous card (←)" className="grid h-9 w-9 shrink-0 cursor-pointer place-items-center rounded-full border border-slate-200 dark:border-white/10 text-heading transition hover:border-teal-200 hover:bg-[#f9fcfc] dark:bg-white/5 disabled:cursor-not-allowed disabled:opacity-30"><ChevronLeft size={16} /></button>

      <div className="flex flex-wrap items-center justify-center gap-2">
        <button type="button" onClick={onAskAI} className="flex cursor-pointer items-center gap-1.5 rounded-full border border-teal-200 bg-teal-50 dark:bg-teal-500/15 dark:text-teal-300 px-3 py-1.5 text-xs font-extrabold text-teal-700 transition hover:bg-teal-100 dark:bg-teal-500/20 dark:text-teal-300"><Sparkles size={13} />Explain with AI</button>
        <button type="button" onClick={onDontUnderstand} className="flex cursor-pointer items-center gap-1.5 rounded-full border border-slate-200 dark:border-white/10 px-3 py-1.5 text-xs font-extrabold text-heading transition hover:border-teal-200 hover:bg-[#f9fcfc] dark:bg-white/5"><MessageCircleQuestion size={13} />I don't understand</button>
        <button type="button" onClick={onBookmarkToggle} className={`flex cursor-pointer items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-extrabold transition ${bookmarked ? "border-amber-300 bg-amber-50 dark:bg-amber-500/15 dark:text-amber-300 text-amber-700" : "border-slate-200 dark:border-white/10 text-heading hover:border-amber-200 hover:bg-amber-50 dark:bg-amber-500/15 dark:text-amber-300"}`}><Bookmark size={13} fill={bookmarked ? "currentColor" : "none"} />Bookmark</button>
      </div>

      <button type="button" onClick={goNext} disabled={atEnd} title="Next card (→)" className="grid h-9 w-9 shrink-0 cursor-pointer place-items-center rounded-full border border-slate-200 dark:border-white/10 text-heading transition hover:border-teal-200 hover:bg-[#f9fcfc] dark:bg-white/5 disabled:cursor-not-allowed disabled:opacity-30"><ChevronRight size={16} /></button>
    </div>
