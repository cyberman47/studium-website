...
function SignupForm() {
  ...
  // Subtle referral acknowledgment only—never a code field the new student
  // has to fill in. Supports both entry points: app/ref/[code] (the real
  // shareable link, already stashed by the time this mounts) and a direct
  // ?ref= query param, in case a link gets shared in that shape instead.
  const [referralCode, setReferralCode] = useState<string | null>(null);

  useEffect(() => {
    const fromQuery = searchParams.get("ref");
    if (fromQuery) capturePendingReferralCode(fromQuery);
    setReferralCode(fromQuery ?? getPendingReferralCode());
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data, error: signUpError } = await supabase.auth.signUp({
      ...
      password,
      options: {
        data: { name, ...(referralCode ? { ref_code: referralCode } : {}) },
