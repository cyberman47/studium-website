...
export default function CreateTerminologyPage() {
  ...
  return <section className="relative py-10 sm:py-14">
    ...
    <div className="relative mt-8 max-w-2xl space-y-5">
      {terms.map(term => <div key={term.id} className="rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-[#0d1917] p-6 shadow-soft sm:p-7">
        <p className="text-xs font-extrabold uppercase tracking-wide text-slate-500">Term</p>
        <h2 className="mt-1 text-xl font-extrabold text-heading">{term.name}</h2>
        ...
        <div className="mt-1.5 flex flex-wrap gap-2">{term.wordBreakdown.map(wp => <span key={wp.part} className="rounded-full border border-slate-200 dark:border-white/10 bg-[#f9fcfc] dark:bg-white/5 px-3 py-1 text-xs font-bold text-heading">{wp.part} <span className="font-medium text-slate-500">= {wp.meaning}</span></span>)}</div>
        ...
        <div className="mt-5 flex flex-wrap gap-2.5">
          ...
          <button type="button" onClick={() => handleCreateFlashcard(term.id)} disabled={flashcardedIds.has(term.id)} className="flex cursor-pointer items-center gap-1.5 rounded-full border border-slate-200 dark:border-white/10 px-4 py-2 text-xs font-extrabold text-heading transition hover:border-teal-200 hover:bg-[#f9fcfc] dark:bg-white/5 disabled:cursor-not-allowed disabled:opacity-60">
            {flashcardedIds.has(term.id) ? "Flashcard created ✓" : "Create Flashcard"}
          </button>
          <button type="button" onClick={() => setAiNoticeId(aiNoticeId === term.id ? null : term.id)} className="flex cursor-pointer items-center gap-1.5 rounded-full border border-slate-200 dark:border-white/10 px-4 py-2 text-xs font-extrabold text-heading transition hover:border-teal-200 hover:bg-[#f9fcfc] dark:bg-white/5"><Wand2 size={13} />Ask AI</button>
        </div>
        {aiNoticeId === term.id && <p className="mt-3 rounded-xl bg-[#f9fcfc] dark:bg-white/5 p-3 text-xs leading-relaxed text-slate-500">Live AI Q&amp;A isn't connected in this demo yet—but here's the written explanation: "{term.aiExplanation}"</p>}
      </div>)}
    </div>
