  ...
  return <section className="relative py-10 sm:py-14">
    ...
    <div className="relative mt-8 max-w-2xl space-y-6">
      <div className="rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-[#0d1917] p-6 shadow-soft sm:p-7">
        ...
        <p className="mt-2 text-base leading-relaxed text-heading">{term.definition}</p>
        ...
        <div className="mt-2.5 flex flex-wrap gap-2">
          {term.wordBreakdown.map(wp => <span key={wp.part} className="rounded-full border border-slate-200 dark:border-white/10 bg-[#f9fcfc] dark:bg-white/5 px-3 py-1.5 text-xs font-bold text-heading">{wp.part} <span className="font-medium text-slate-500">= {wp.meaning}</span></span>)}
        </div>

        {relatedTerms.length > 0 && <>
          <h2 className="mt-6 text-xs font-extrabold uppercase tracking-wide text-slate-500">Related Terms</h2>
          <div className="mt-2.5 flex flex-wrap gap-2">
            {relatedTerms.map(rt => <Link key={rt.id} href={`/dashboard/terminology/${rt.categoryId}/${rt.id}`} className="cursor-pointer rounded-full bg-teal-50 dark:bg-teal-500/15 dark:text-teal-300 px-3 py-1.5 text-xs font-extrabold text-teal-700 transition hover:bg-teal-100 dark:bg-teal-500/20 dark:text-teal-300">{rt.name}</Link>)}
          </div>
        </>}

        <div className="mt-6 rounded-2xl bg-[#f9fcfc] dark:bg-white/5 p-4">
