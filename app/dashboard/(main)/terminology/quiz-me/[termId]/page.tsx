...
export default function QuizMeOnTermPage({ params }: { params: { termId: string } }) {
  ...
  return <section className="relative py-10 sm:py-14">
    ...
    {matched.length > 0
      ? <div className="mt-6 max-w-lg rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-[#0d1917] p-6 shadow-soft sm:p-7">
        <p className="text-sm leading-relaxed text-slate-600">Studium found <span className="font-extrabold text-heading">{matched.length} real practice question{matched.length === 1 ? "" : "s"}</span> related to this term.</p>
      ...
      : <div className="mt-6 max-w-lg rounded-3xl border border-dashed border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d1917] p-6 text-center shadow-soft sm:p-7">
        <p className="text-sm font-extrabold text-heading">No practice questions have been written for this term yet.</p>
        ...
        <Link href="/dashboard/learning-paths/mcat/practice" className="mt-4 inline-flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 dark:border-white/10 px-5 py-2.5 text-sm font-bold text-heading transition hover:-translate-y-0.5 hover:border-teal-200 hover:bg-[#f9fcfc] dark:bg-white/5">Browse Real Practice Questions</Link>
      </div>}
  </section>;
}
