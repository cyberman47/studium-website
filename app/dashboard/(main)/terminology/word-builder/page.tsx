      ...
      {wordRoots.map((root, i) => <Reveal key={root.id} delay={i * 0.03}>
        <div className="flex h-full flex-col rounded-3xl border border-slate-100 bg-white p-5 shadow-soft">
          <div className="flex items-center gap-2">
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-teal-100 text-sm font-extrabold text-teal-700"><Sparkles size={16} /></span>
            <div className="min-w-0">
              <p className="text-base font-extrabold text-ink">{root.part}</p>
              <p className="text-xs text-slate-500">= {root.meaning}</p>
            </div>
          </div>
          <div className="mt-4 flex flex-wrap gap-1.5">
            {root.examples.map(ex => <span key={ex} className="rounded-full bg-[#f9fcfc] px-2.5 py-1 text-xs font-bold text-ink">{ex}</span>)}
          </div>
        </div>
      </Reveal>)}
    </div>
