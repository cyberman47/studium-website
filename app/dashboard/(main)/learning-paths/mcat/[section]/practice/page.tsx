...
export default function MCATSectionPracticePage({ params }: { params: { section: string } }) {
  ...
  return <section className="relative py-10 sm:py-14">
    ...
    <nav className="mt-6 flex flex-wrap gap-2">
      <button type="button" onClick={() => setTab("quiz")} className={`inline-flex cursor-pointer items-center gap-1.5 rounded-full border px-4 py-2 text-xs font-extrabold transition ${tab === "quiz" ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200 hover:bg-[#f9fcfc]"}`}><ListChecks size={13} />Build a Quiz</button>
      <button type="button" onClick={() => setTab("flashcards")} className={`inline-flex cursor-pointer items-center gap-1.5 rounded-full border px-4 py-2 text-xs font-extrabold transition ${tab === "flashcards" ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200 hover:bg-[#f9fcfc]"}`}><Layers size={13} />Build a Flashcard Deck</button>
    </nav>

    <div className="mt-8">
      {tab === "quiz"
        ? <div className="max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-soft sm:p-7">
          ...
          {mounted && <>
            ...
            <div className="mt-5">
              ...
              {usedFallback && <div className="mt-2.5 flex items-start gap-2 rounded-xl border border-teal-200 bg-teal-50 p-3">
                ...
                <p className="text-[11px] font-semibold leading-relaxed text-teal-800">You&apos;ve completed all unused questions in this section! Automatically including previously answered questions so you can review.</p>
              </div>}
            </div>
        ...
        : <div className="max-w-3xl">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-soft sm:p-7">
            <span className="grid h-12 w-12 place-items-center rounded-2xl bg-violet-100 text-violet-600"><Layers size={22} /></span>
            <h2 className="mt-5 text-lg font-extrabold tracking-tight text-ink">Build a Flashcard Deck</h2>
            ...
            {mounted && sectionCards.length > 0 && <>
              <div className="mt-5 flex flex-wrap items-center gap-2">
                <input
                  ...
                  className="min-w-[180px] flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-ink outline-none focus:border-teal-400"
                />
                <button type="button" onClick={() => setCardSubjectFilter(null)} className={`cursor-pointer rounded-full border px-3 py-1.5 text-xs font-bold transition ${!cardSubjectFilter ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>All subjects</button>
                {cardSubjects.map(s => <button key={s} type="button" onClick={() => setCardSubjectFilter(s)} className={`cursor-pointer rounded-full border px-3 py-1.5 text-xs font-bold transition ${cardSubjectFilter === s ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>{s}</button>)}
              </div>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                {selectedCardIds.size > 0 && <button type="button" onClick={clearCardSelection} className="cursor-pointer text-xs font-bold text-slate-400 hover:text-ink">Clear selection</button>}
              </div>
                ...
                {visibleCards.length === 0
                  ...
                  : visibleCards.map((card: LibraryCard) => {
                    ...
                    return <button
                      ...
                      className={`flex w-full cursor-pointer items-start gap-3 rounded-xl border p-3 text-left transition ${isSelected ? "border-teal-300 bg-teal-50/60" : "border-slate-100 bg-white hover:border-teal-200 hover:bg-[#f9fcfc]"}`}
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-sm font-bold text-ink">{card.front}</span>
          ...
          {selectedCardIds.size > 0 && <div className="sticky bottom-6 mt-5 flex flex-wrap items-center gap-3 rounded-full border border-slate-200 bg-white px-5 py-3 shadow-lift">
            <span className="text-sm font-bold text-ink">{selectedCardIds.size} card{selectedCardIds.size === 1 ? "" : "s"} selected</span>
            <button type="button" onClick={studySelectedNow} className="inline-flex cursor-pointer items-center gap-1.5 rounded-full border border-slate-200 px-4 py-2 text-xs font-bold text-ink transition hover:border-teal-300 hover:bg-[#f9fcfc]"><Layers size={13} />Study Now</button>
            ...
            <button type="button" onClick={clearCardSelection} className="cursor-pointer text-slate-400 hover:text-ink" aria-label="Clear selection"><X size={16} /></button>
          </div>}
