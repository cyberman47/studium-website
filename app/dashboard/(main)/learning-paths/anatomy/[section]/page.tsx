...
import { findAnatomySection } from "@/lib/anatomyPath";
...
export default function AnatomySectionPage({ params }: { params: { section: string } }) {
  const section = findAnatomySection(params.section);

  if (!section) {
    return <section className="relative py-10 sm:py-14">
      <p className="text-sm text-slate-500">Section not found.</p>
      <Link href="/dashboard/learning-paths/anatomy" className="mt-3 inline-block cursor-pointer text-sm font-bold text-teal-600 hover:text-teal-700">← Back to Anatomy path</Link>
    </section>;
  }
  ...
  return <section className="relative py-10 sm:py-14">
    ...
    <Link href="/dashboard/learning-paths/anatomy" className="mb-4 inline-flex cursor-pointer items-center gap-2 text-xs font-bold text-slate-500 transition hover:text-teal-600">← Back to Anatomy path</Link>
    ...
    <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {section.subjects.map((subject, i) => <Reveal key={subject.id} delay={i * 0.03}>
