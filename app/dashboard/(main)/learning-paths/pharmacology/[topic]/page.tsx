...
import { findPharmacologyTopic } from "@/lib/pharmacologyPath";
...
export default function PharmacologyTopicPage({ params }: { params: { topic: string } }) {
  const topic = findPharmacologyTopic(params.topic);

  if (!topic) {
    return <section className="relative py-10 sm:py-14">
      <p className="text-sm text-slate-500">Topic not found.</p>
      <Link href="/dashboard/learning-paths/pharmacology" className="mt-3 inline-block cursor-pointer text-sm font-bold text-teal-600 hover:text-teal-700">← Back to Pharmacology path</Link>
    </section>;
  }
  ...
  return <section className="relative py-10 sm:py-14">
    ...
    <Link href="/dashboard/learning-paths/pharmacology" className="mb-4 inline-flex cursor-pointer items-center gap-2 text-xs font-bold text-slate-500 transition hover:text-teal-600">← Back to Pharmacology path</Link>
