...
const tileColors = [
  "bg-violet-100 dark:bg-violet-500/20 dark:text-violet-300 text-violet-600", "bg-teal-100 dark:bg-teal-500/20 dark:text-teal-300 text-teal-700", "bg-red-100 text-red-600", "bg-amber-100 dark:bg-amber-500/20 dark:text-amber-300 text-amber-600",
  "bg-sky-100 dark:bg-sky-500/20 dark:text-sky-300 text-sky-600", "bg-lime-100 dark:bg-lime-500/20 dark:text-lime-300 text-lime-700", "bg-pink-100 dark:bg-pink-500/20 dark:text-pink-300 text-pink-600", "bg-slate-200 text-slate-600"
];
    ...
    <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {pharmacologyTopics.map((topic, i) => {
        ...
        return <Reveal key={topic.id} delay={i * 0.03}>
          <Link href={`/dashboard/learning-paths/pharmacology/${topic.id}`} className="group flex h-full flex-col rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-[#0d1917] p-6 shadow-soft transition hover:-translate-y-0.5 hover:shadow-lift">
