...
import { findMedicalSchoolTopic } from "@/lib/medicalSchoolPath";
...
export default function MedicalSchoolTopicPage({ params }: { params: { topic: string } }) {
  const topic = findMedicalSchoolTopic(params.topic);

  if (!topic || topic.externalHref) {
    return <section className="relative py-10 sm:py-14">
      <p className="text-sm text-slate-500">Topic not found.</p>
      <Link href="/dashboard/learning-paths/medical-school" className="mt-3 inline-block cursor-pointer text-sm font-bold text-teal-600 hover:text-teal-700">← Back to Medical School path</Link>
    </section>;
  }
  ...
  return <section className="relative py-10 sm:py-14">
    ...
    <Link href="/dashboard/learning-paths/medical-school" className="mb-4 inline-flex cursor-pointer items-center gap-2 text-xs font-bold text-slate-500 transition hover:text-teal-600">← Back to Medical School path</Link>
