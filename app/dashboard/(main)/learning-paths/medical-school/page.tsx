...
const tileColors = [
  "bg-red-100 text-red-600", "bg-indigo-100 text-indigo-600", "bg-slate-200 text-slate-600", "bg-amber-100 text-amber-600",
  "bg-lime-100 text-lime-700", "bg-sky-100 text-sky-600", "bg-emerald-100 text-emerald-600", "bg-pink-100 text-pink-600",
    ...
    <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {medicalSchoolTopics.map((topic, i) => {
        ...
        return <Reveal key={topic.id} delay={i * 0.03}>
          <Link href={href} className="group flex h-full flex-col rounded-3xl border border-slate-100 bg-white p-6 shadow-soft transition hover:-translate-y-0.5 hover:shadow-lift">
