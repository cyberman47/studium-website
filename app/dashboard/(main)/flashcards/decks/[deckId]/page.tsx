...
import {
  CardSource, getCardDisplayStatus, getCardProgress, getLibraryCardsByIds, LibraryCard, reviewLibraryCard,
  restoreLibraryCard, sortLabels, SortOption, sortLibraryCards, CARD_PROGRESS_EVENT
...
export default function DeckDetailPage({ params }: { params: { deckId: string } }) {
  ...
  if (studyQueue) {
    ...
    return <FlashcardFocusMode
      ...
      onRate={handleRate}
      onUndo={handleUndo}
      tutorContext={deck ? { chatKey: `deck:${deck.id}`, subjectName: deck.name } : undefined}
    />;
  }
  ...
  return <section className="relative py-10 sm:py-14">
    ...
    {renaming
      ? <div className="mt-5 flex items-center gap-2">
        <input autoFocus value={nameDraft} onChange={e => setNameDraft(e.target.value)} className="display w-full max-w-lg rounded-2xl border border-slate-200 px-4 py-2 text-3xl leading-tight outline-none focus:border-teal-400 sm:text-4xl" />
        ...
        <button type="button" onClick={() => setRenaming(false)} className="cursor-pointer rounded-full border border-slate-200 p-2.5 text-slate-500 hover:text-ink"><X size={16} /></button>
      </div>
    ...
    <div className="mt-6 flex flex-wrap gap-3">
      ...
      {stats && stats.due > 0 && <button type="button" onClick={() => startStudy(deck.name, cards)} className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 px-6 py-3 text-sm font-bold text-ink transition hover:border-teal-200 hover:bg-[#f9fcfc]">Study All ({stats.total})</button>}
      {stats && stats.new > 0 && <button type="button" onClick={() => startStudy(deck.name, cards.filter(c => getCardDisplayStatus(c.id) === "new"))} className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 px-6 py-3 text-sm font-bold text-ink transition hover:border-teal-200 hover:bg-[#f9fcfc]">Study New ({stats.new})</button>}
      {stats && stats.learning > 0 && <button type="button" onClick={() => startStudy(deck.name, cards.filter(c => getCardDisplayStatus(c.id) === "learning"))} className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 px-6 py-3 text-sm font-bold text-ink transition hover:border-teal-200 hover:bg-[#f9fcfc]">Continue Learning ({stats.learning})</button>}
      <Link href={`/dashboard/flashcards/create?deckId=${deck.id}`} className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 px-6 py-3 text-sm font-bold text-ink transition hover:border-teal-200 hover:bg-[#f9fcfc]"><Plus size={16} />Add Cards</Link>
    </div>

    <div className="mt-3 flex flex-wrap gap-2">
      <button type="button" onClick={() => setMerging(true)} className="inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold text-slate-500 transition hover:bg-slate-100 hover:text-ink">Merge decks</button>
      <button type="button" onClick={() => router.push(`/dashboard/flashcards/decks/${duplicateDeck(deck!.id)?.id}`)} className="inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold text-slate-500 transition hover:bg-slate-100 hover:text-ink"><Copy size={13} />Duplicate</button>
      {deck.archived
        ? <button type="button" onClick={() => unarchiveDeck(deck!.id)} className="inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold text-slate-500 transition hover:bg-slate-100 hover:text-ink"><Archive size={13} />Unarchive</button>
        : <button type="button" onClick={() => archiveDeck(deck!.id)} className="inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold text-slate-500 transition hover:bg-slate-100 hover:text-ink"><Archive size={13} />Archive</button>}
      <button
        type="button"
        ...
        className="inline-flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold text-rose-600 transition hover:bg-rose-50"
      ><Trash2 size={13} />Delete deck</button>
    </div>

    {merging && <div className="mt-6 max-w-md rounded-3xl border border-slate-200 bg-white p-5 shadow-soft">
      <p className="text-sm font-extrabold text-ink">Merge into a new deck</p>
      ...
      <div className="mt-3 max-h-40 space-y-1.5 overflow-y-auto">
        ...
        {otherDecks.map(d => {
          ...
          return <button key={d.id} type="button" onClick={() => setMergeSelection(prev => { const next = new Set(prev); if (next.has(d.id)) next.delete(d.id); else next.add(d.id); return next; })} className={`flex w-full cursor-pointer items-center justify-between rounded-xl border px-3 py-2 text-left text-sm font-bold transition ${checked ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-100 text-ink hover:border-teal-200"}`}>
      ...
      <div className="mt-4 flex justify-end gap-2">
        <button type="button" onClick={() => { setMerging(false); setMergeSelection(new Set()); }} className="cursor-pointer rounded-full px-4 py-2 text-xs font-bold text-slate-500 hover:text-ink">Cancel</button>
        <button
          type="button"
    ...
    {cards.length === 0
      ? <div className="mt-8 rounded-3xl border border-dashed border-slate-200 bg-white p-10 text-center shadow-soft">
        <span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-teal-100 text-teal-700"><Layers size={26} /></span>
        <p className="mt-4 text-base font-extrabold text-ink">This deck is empty.</p>
      ...
      : <>
        <div className="mt-8 flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setFilter("all")} className={`cursor-pointer rounded-full border px-3.5 py-1.5 text-xs font-extrabold transition ${filter === "all" ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>All cards</button>
            {statusOrder.map(s => <button key={s} type="button" onClick={() => setFilter(filter === s ? "all" : s)} className={`flex cursor-pointer items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-xs font-extrabold transition ${filter === s ? `${statusStyles[s].border} ${statusStyles[s].bg} ${statusStyles[s].text}` : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>
        ...
        <div className="mt-4 space-y-1.5 rounded-3xl border border-slate-100 bg-white p-2 shadow-soft">
          ...
          {filtered.map(card => {
            ...
            return <div key={card.id} className={`flex items-center gap-3 rounded-2xl border px-3 py-2.5 transition ${style.border} ${style.bg}`}>
              ...
              <button type="button" onClick={() => removeCardFromDeck(deck!.id, card.id)} className="shrink-0 cursor-pointer rounded-full p-1.5 text-slate-300 hover:bg-rose-50 hover:text-rose-600" aria-label="Remove from deck"><X size={15} /></button>
            </div>;
          })}
        </div>
      </>}
