...
function MethodTile({ icon: Icon, title, description, onClick, comingSoon, reason }: {
  ...
  if (comingSoon) return <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-6 opacity-80">
    <div className="flex items-start justify-between gap-3">
      <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-slate-100 text-slate-400"><Icon size={20} /></span>
      <span className="shrink-0 rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wide text-amber-700">Coming Soon</span>
    </div>
  ...
  return <button type="button" onClick={onClick} className="group cursor-pointer rounded-3xl border border-slate-100 bg-white p-6 text-left shadow-soft transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-lift">
    <div className="flex items-start justify-between gap-3">
      <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-teal-50 text-teal-600"><Icon size={20} /></span>
...
function TerminologyMethod({ onSubmit }: { onSubmit: (ids: string[]) => void }) {
  ...
  return <div className="mt-8">
    <input
      ...
      className="w-full rounded-full border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-ink outline-none focus:border-teal-400"
    />
    <div className="mt-3 flex flex-wrap gap-2">
      <button type="button" onClick={() => setConfidence(null)} className={`cursor-pointer rounded-full border px-3.5 py-1.5 text-xs font-extrabold transition ${!confidence ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>All ({terms.length})</button>
      {(["dont-know", "somewhat", "know-well"] as ConfidenceLevel[]).map(c => <button key={c} type="button" onClick={() => setConfidence(confidence === c ? null : c)} className={`flex cursor-pointer items-center gap-1.5 rounded-full border px-3.5 py-1.5 text-xs font-extrabold transition ${confidence === c ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}><span className={`h-1.5 w-1.5 rounded-full ${confidenceDots[c]}`} />{confidenceLabels[c]}</button>)}
      <button type="button" onClick={() => setConfidence(confidence === "unrated" ? null : "unrated")} className={`cursor-pointer rounded-full border px-3.5 py-1.5 text-xs font-extrabold transition ${confidence === "unrated" ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>Not rated yet</button>
    </div>

    <div className="mt-4 max-h-[440px] space-y-1.5 overflow-y-auto rounded-3xl border border-slate-100 bg-white p-2 shadow-soft">
      {filtered.length === 0 && <p className="py-16 text-center text-sm text-slate-400">No terms match these filters.</p>}
      {filtered.map(t => {
        const isSelected = selected.has(t.id);
        return <button key={t.id} type="button" onClick={() => toggle(t.id)} className={`flex w-full cursor-pointer items-center gap-3 rounded-2xl px-3 py-2.5 text-left transition ${isSelected ? "bg-teal-50" : "hover:bg-slate-50"}`}>
...
function ContentMethod({ onSubmit }: { onSubmit: (ids: string[]) => void }) {
  ...
  return <div className="mt-8">
    <div className="flex flex-wrap gap-2">
      {mcatSections.map(s => <button key={s.id} type="button" onClick={() => selectSection(s.id)} className={`cursor-pointer rounded-full border px-3.5 py-1.5 text-xs font-extrabold transition ${sectionId === s.id ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>{s.shortTitle}</button>)}
    </div>

    {section && <div className="mt-3 flex flex-wrap gap-1.5">
      {section.subjects.map(s => <button key={s.id} type="button" onClick={() => selectSubject(s.id)} className={`cursor-pointer rounded-full border px-3 py-1 text-[11px] font-bold transition ${subjectId === s.id ? "border-teal-400 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>{s.name}</button>)}
    </div>}
    ...
    {subject && <div className="mt-4 space-y-1.5">
      {subject.lessons.map(l => {
        ...
        return <button
          ...
          className={`flex w-full items-center justify-between rounded-2xl border px-4 py-3 text-left text-sm font-bold transition ${!available ? "cursor-not-allowed border-slate-100 text-slate-300" : lessonId === l.id ? "cursor-pointer border-teal-500 bg-teal-50 text-teal-700" : "cursor-pointer border-slate-100 text-ink hover:border-teal-200"}`}
        >
          <span>{l.title}</span>
    ...
    {lessonContent && <>
      <div className="mt-5 max-h-[380px] space-y-1.5 overflow-y-auto rounded-3xl border border-slate-100 bg-white p-2 shadow-soft">
        {lessonContent.flashcards.map((fc, i) => {
          const isSelected = selected.has(i);
          return <button key={i} type="button" onClick={() => toggle(i)} className={`flex w-full cursor-pointer items-start gap-3 rounded-2xl px-3 py-2.5 text-left transition ${isSelected ? "bg-teal-50" : "hover:bg-slate-50"}`}>
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-bold text-ink">{fc.front}</p>
...
function ManualMethod({ onSubmit }: { onSubmit: (ids: string[]) => void }) {
  ...
  return <div className="mt-8 space-y-3">
    {cards.map((c, i) => <div key={i} className="rounded-3xl border border-slate-100 bg-white p-4 shadow-soft">
      <div className="flex items-center justify-between">
        <p className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Card {i + 1}</p>
        <div className="flex items-center gap-1">
          <button type="button" onClick={() => duplicateCard(i)} title="Duplicate" className="cursor-pointer rounded-full p-1.5 text-slate-300 transition hover:bg-slate-100 hover:text-ink"><Copy size={14} /></button>
          <button type="button" onClick={() => deleteCard(i)} disabled={cards.length === 1} title="Delete" className="cursor-pointer rounded-full p-1.5 text-slate-300 transition hover:bg-rose-50 hover:text-rose-600 disabled:cursor-not-allowed disabled:opacity-30"><Trash2 size={14} /></button>
        </div>
      </div>
      <textarea value={c.front} onChange={e => update(i, "front", e.target.value)} placeholder="Front—question or term" rows={2} className="mt-2 w-full resize-none rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-ink outline-none focus:border-teal-400" />
      <textarea value={c.back} onChange={e => update(i, "back", e.target.value)} placeholder="Back—answer or definition" rows={2} className="mt-2 w-full resize-none rounded-xl border border-slate-200 px-3 py-2 text-sm text-ink outline-none focus:border-teal-400" />
    </div>)}

    <button type="button" onClick={addCard} className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-slate-200 py-3 text-sm font-bold text-slate-500 transition hover:border-teal-300 hover:text-teal-600"><Plus size={15} />Add another card</button>
