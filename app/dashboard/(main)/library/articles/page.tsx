...
export default function ArticlesPage() {
  ...
  return <section className="relative py-10 sm:py-14">
    ...
    {topics.length > 0 && <div className="mt-8 flex flex-wrap items-center gap-2">
      <button type="button" onClick={() => setTopic(null)} className={`cursor-pointer rounded-full border px-3.5 py-1.5 text-xs font-extrabold transition ${!topic ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>All topics</button>
      {topics.map(t => <button key={t} type="button" onClick={() => setTopic(topic === t ? null : t)} className={`cursor-pointer rounded-full border px-3.5 py-1.5 text-xs font-extrabold transition ${topic === t ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-500 hover:border-teal-200"}`}>{t}</button>)}
    </div>}

    <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {filtered.map(a => {
        const saved = isInLibrary("article", a.id);
        return <div key={a.id} className="flex flex-col rounded-3xl border border-slate-200 bg-white p-5 shadow-soft transition hover:-translate-y-0.5 hover:shadow-lift">
          <div className="flex items-start justify-between gap-2">
            <p className="text-[11px] font-extrabold uppercase tracking-wide text-amber-600">{a.topic}</p>
            <button type="button" onClick={() => toggleLibrarySave("article", a.id)} title={saved ? "Remove from Library" : "Save to Library"} className={`grid h-8 w-8 shrink-0 cursor-pointer place-items-center rounded-full border transition ${saved ? "border-teal-500 bg-teal-50 text-teal-600" : "border-slate-200 text-slate-400 hover:border-teal-200 hover:text-teal-600"}`}><Bookmark size={14} fill={saved ? "currentColor" : "none"} /></button>
          </div>
          <Link href={`/dashboard/library/articles/${a.id}`} className="mt-1.5 cursor-pointer text-base font-extrabold tracking-tight text-ink hover:text-teal-700">{a.title}</Link>
