...
export default function LibraryPage() {
  ...
  return <section className="relative py-10 sm:py-14">
    ...
    <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {categories.map((c, i) => <Link key={c.href} href={c.href} className="group flex cursor-pointer flex-col rounded-3xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d1917] p-6 shadow-soft transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-lift" style={{ transitionDelay: `${i * 15}ms` }}>
        <div className="flex items-start justify-between gap-3">
          <span className="grid h-11 w-11 place-items-center rounded-2xl bg-teal-50 dark:bg-teal-500/15 dark:text-teal-300 text-teal-600"><c.icon size={20} /></span>
          <ArrowRight size={16} className="mt-2 shrink-0 text-slate-300 transition group-hover:translate-x-1 group-hover:text-teal-500" />
        </div>
        <h3 className="mt-4 text-base font-extrabold tracking-tight text-heading">{c.label}</h3>
    ...
    <div className="mt-10">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-3">
        <Link href="/dashboard/flashcards" className="flex cursor-pointer items-center justify-between rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d1917] p-4 transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-soft">
          <div><p className="text-sm font-extrabold text-heading">My Decks</p><p className="mt-0.5 text-xs text-slate-500">{counts.decks} deck{counts.decks === 1 ? "" : "s"}</p></div>
          <ArrowRight size={15} className="text-slate-300" />
        </Link>
        <Link href="/dashboard/library/saved#flagged" className="flex cursor-pointer items-center justify-between rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d1917] p-4 transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-soft">
          <div><p className="text-sm font-extrabold text-heading">Flagged Questions</p><p className="mt-0.5 text-xs text-slate-500">{counts.flagged} flagged</p></div>
          <ArrowRight size={15} className="text-slate-300" />
        </Link>
        <Link href="/dashboard/library/saved#mistakes" className="flex cursor-pointer items-center justify-between rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d1917] p-4 transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-soft">
          <div className="flex items-center gap-2.5"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-rose-100 dark:bg-rose-500/20 dark:text-rose-300 text-rose-600"><TriangleAlert size={14} /></span><div><p className="text-sm font-extrabold text-heading">Mistake Vault</p><p className="mt-0.5 text-xs text-slate-500">{counts.missed} to review</p></div></div>
          <ArrowRight size={15} className="text-slate-300" />
        </Link>
      </div>
    </div>
...
function CalendarView({ month, setMonth, attemptsByDate, todayKey, streak, onSelectDay, onClose }: {
  ...
  return <>
    <div className="flex items-center justify-between">
      <div>
        <h3 className="text-base font-extrabold text-heading">Past Cases Archive</h3>
      ...
      <button type="button" onClick={onClose} className="cursor-pointer rounded-full p-1.5 text-slate-400 hover:bg-slate-100 dark:bg-white/10 hover:text-heading" aria-label="Close"><X size={16} /></button>
    </div>

    <div className="mt-5 flex items-center justify-between">
      <button type="button" onClick={() => setMonth(new Date(year, monthIndex - 1, 1))} className="grid h-8 w-8 cursor-pointer place-items-center rounded-full text-slate-400 hover:bg-slate-100 dark:bg-white/10 hover:text-heading" aria-label="Previous month"><ChevronLeft size={16} /></button>
      <p className="text-sm font-extrabold text-heading">{month.toLocaleDateString("en-US", { month: "long", year: "numeric" })}</p>
      <button type="button" onClick={() => setMonth(new Date(year, monthIndex + 1, 1))} className="grid h-8 w-8 cursor-pointer place-items-center rounded-full text-slate-400 hover:bg-slate-100 dark:bg-white/10 hover:text-heading" aria-label="Next month"><ChevronRight size={16} /></button>
    </div>
    ...
    <div className="mt-1.5 grid grid-cols-7 gap-1.5">
      {cells.map((key, i) => {
        ...
        return <button
          ...
          className={`flex aspect-square cursor-pointer flex-col items-center justify-center gap-1 rounded-xl text-xs font-bold transition ${isToday ? "ring-2 ring-teal-500" : ""} ${attempt ? "hover:bg-slate-50 dark:bg-white/5" : "cursor-default"} ${isFuture ? "text-slate-300" : "text-heading"}`}
        >
          {day}
    ...
    <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-slate-100 dark:border-white/10 pt-4 text-[11px] font-bold text-slate-500">
...
function CaseReview({ dateKey, attempt, onBack, onClose }: { dateKey: string; attempt: CaseAttempt; onBack: () => void; onClose: () => void }) {
  ...
  return <>
    ...
    <h3 className="mt-1 text-lg font-extrabold text-heading">{caseForDay.title}</h3>
    ...
    <div className="mt-4 space-y-2">
      {caseForDay.options.map((option, i) => {
        ...
        return <div key={i} className={`flex items-center justify-between gap-3 rounded-2xl border px-4 py-3 text-sm font-bold text-heading ${isCorrect ? "border-teal-500 bg-teal-50 dark:bg-teal-500/15 dark:text-teal-300" : isSelected ? "border-rose-300 bg-rose-50 dark:bg-rose-500/15 dark:text-rose-300" : "border-slate-100 dark:border-white/10 opacity-60"}`}>
          <span>{option}</span>
