  ...
  return <section className="relative py-10 sm:py-14">
    ...
    <div className="mt-8 space-y-3">
      {resources.map(r => {
        ...
        return <div key={r.id} className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-soft transition hover:-translate-y-0.5 hover:shadow-lift">
          ...
          <div className="min-w-0 flex-1">
            ...
            <a href={r.url} target="_blank" rel="noreferrer" className="mt-0.5 inline-flex cursor-pointer items-center gap-1.5 text-sm font-extrabold text-ink hover:text-teal-700">{r.title}<ExternalLink size={12} className="text-slate-400" /></a>
            <p className="mt-1 text-xs leading-relaxed text-slate-500">{r.description}</p>
          </div>
          <button type="button" onClick={() => toggleLibrarySave("resource", r.id)} title={saved ? "Remove from Library" : "Save to Library"} className={`grid h-9 w-9 shrink-0 cursor-pointer place-items-center rounded-full border transition ${saved ? "border-teal-500 bg-teal-50 text-teal-600" : "border-slate-200 text-slate-400 hover:border-teal-200 hover:text-teal-600"}`}><Bookmark size={15} fill={saved ? "currentColor" : "none"} /></button>
        </div>;
      })}
    </div>
  </section>;
