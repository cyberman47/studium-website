      ...
      <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-soft sm:p-7">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          {languages.map(l => {
            ...
            return <button type="button" key={l.code} onClick={() => choose(l)} className={`flex cursor-pointer items-center justify-between gap-2 rounded-2xl border px-4 py-3 text-left text-sm font-bold transition ${active ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-600 hover:border-teal-200"}`}>
