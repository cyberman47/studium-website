      ...
      <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-soft sm:p-7">
        ...
        <div className="mt-5">
          ...
          <div className="flex gap-2">
            {sessionSizes.map(size => <button type="button" key={size} onClick={() => update({ cardsPerSession: size })} className={`cursor-pointer rounded-full border px-4 py-1.5 text-xs font-bold transition ${settings.cardsPerSession === size ? "border-teal-500 bg-teal-50 text-teal-700" : "border-slate-200 text-slate-600 hover:border-teal-200"}`}>{size}</button>)}
          </div>
        </div>
