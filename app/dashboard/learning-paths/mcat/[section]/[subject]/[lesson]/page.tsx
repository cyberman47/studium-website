...
export default function MCATLessonPage({ params }: { params: { section: string; subject: string; lesson: string } }) {
  ...
  useEffect(() => {
    ...
    if (step === "flashcards" && !tipsShown.has("flashcards-takeaway") && lesson.keyTakeaways.length > 0) {
      setProactiveTip({ icon: "💡", text: lesson.keyTakeaways[0], sourceLabel: "From this lesson's key takeaways" });
    ...
    if (step === "practice" && !tipsShown.has("practice-connection") && lesson.sections[0]?.keyTerms[0]) {
      ...
      setProactiveTip({ icon: "🔗", text: `This connects to "${kt.term}": ${kt.definition}`, sourceLabel: "From this lesson's key terms" });
  ...
  useEffect(() => {
    ...
    if (lastTwo.every(r => !r.correct) && !tipsShown.has("miss-streak")) {
      setProactiveTip({ icon: "⚠️", text: "You've missed the last two in a row—ask your tutor for a hint before continuing.", sourceLabel: "Based on your recent answers" });
  ...
  return <div className="relative flex bg-white" style={{ minHeight: "calc(100vh - 89px)" }}>
    ...
    <section className={`min-w-0 flex-1 px-6 py-10 transition-[max-width] duration-300 sm:px-10 sm:py-14 lg:mx-auto ${panelOpen ? "lg:max-w-3xl" : "lg:max-w-5xl"}`} onMouseUp={step === "learn" ? handleMouseUp : undefined}>
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {step !== "hub" && <button type="button" onClick={() => setStep("hub")} className="flex cursor-pointer items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-ink shadow-soft transition hover:border-teal-200 hover:bg-[#f9fcfc]"><LayoutGrid size={13} />Study Hub</button>}
        </div>
      </div>
      ...
      <div className="max-w-none">
        ...
        {step !== "hub" && <div className="mt-3 h-1.5 w-full max-w-sm overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-teal-500 transition-all" style={{ width: `${realProgressPercent}%` }} /></div>}
      </div>
      ...
      <div className="mt-8 max-w-none">
        ...
        {step === "hub" && <div className="rounded-3xl border border-slate-100 bg-white p-7 shadow-soft sm:p-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-wide text-slate-400">Your progress on this topic</p>
              <p className="mt-1 text-3xl font-extrabold text-ink">{realProgressPercent}%</p>
            </div>
            <div className="h-2 w-full max-w-[220px] flex-1 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-teal-500 transition-all" style={{ width: `${realProgressPercent}%` }} /></div>
          </div>

          <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <button type="button" onClick={() => enterMode("learn")} className="flex cursor-pointer flex-col items-center gap-2 rounded-2xl border border-slate-200 bg-white py-5 text-sm font-bold text-ink transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-soft">
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-teal-100 text-teal-700"><BookOpen size={18} /></span>Learn
            </button>
            <button type="button" onClick={() => enterMode("flashcards")} className="flex cursor-pointer flex-col items-center gap-2 rounded-2xl border border-slate-200 bg-white py-5 text-sm font-bold text-ink transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-soft">
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-sky-100 text-sky-700"><Layers size={18} /></span>Flashcards
            </button>
            <button type="button" onClick={() => enterMode("practice")} className="flex cursor-pointer flex-col items-center gap-2 rounded-2xl border border-slate-200 bg-white py-5 text-sm font-bold text-ink transition hover:-translate-y-0.5 hover:border-teal-200 hover:shadow-soft">
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-violet-100 text-violet-700"><Target size={18} /></span>Practice
            </button>
            <button type="button" onClick={() => setShowWeakAreas(s => !s)} className={`flex cursor-pointer flex-col items-center gap-2 rounded-2xl border py-5 text-sm font-bold transition hover:-translate-y-0.5 hover:shadow-soft ${showWeakAreas ? "border-amber-300 bg-amber-50 text-amber-800" : "border-slate-200 bg-white text-ink hover:border-amber-200"}`}>
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-amber-100 text-amber-700"><AlertTriangle size={18} /></span>Weak Areas
            </button>
          </div>

          <div className="mt-6 flex flex-wrap items-center gap-4 rounded-2xl bg-[#f9fcfc] p-5">
            <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white text-teal-600 shadow-soft"><Sparkles size={17} /></span>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-extrabold text-ink">{recommendation.label}</p>
          ...
          {showWeakAreas && <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50/60 p-5">
            ...
            {hubWeakConcepts.length === 0
              ...
              : <div className="mt-3 space-y-2.5">
                {hubWeakConcepts.map(w => <div key={w.concept} className="flex items-center justify-between gap-3 rounded-xl bg-white px-4 py-2.5 shadow-soft">
                  <span className="text-sm font-bold text-ink">{w.concept}</span>
        ...
        {step === "learn" && <div>
          <article className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm sm:p-12">
            {lesson.sections.map((sec, i) => <div key={sec.heading} className={i > 0 ? "mt-9 border-t border-slate-100 pt-9" : ""}>
              <h2 className="font-display text-xl font-extrabold tracking-tight text-ink">{sec.heading}</h2>
              <p className="mt-3 select-text text-[15px] leading-[1.85] text-slate-700"><InteractiveText text={sec.body} /></p>
              {sec.keyTerms.length > 0 && <dl className="mt-4 rounded-xl bg-[#f9fcfc] p-4">
            ...
            <div className="mt-10 rounded-2xl border-l-4 border-teal-500 bg-teal-50/70 p-6">
              <h3 className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-wide text-teal-800"><Lightbulb size={14} />Key Takeaways</h3>
              <ul className="mt-3 space-y-2.5">
                {lesson.keyTakeaways.map(t => <li key={t} className="flex items-start gap-2.5 text-sm leading-relaxed text-teal-900"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-teal-600" />{t}</li>)}
              </ul>
            </div>
          </article>
      ...
      <AnimatePresence>
        ...
        >
          <div className="flex items-start justify-between gap-2">
            ...
            <button type="button" onClick={() => setConnectedTerms(null)} className="cursor-pointer text-slate-400 hover:text-ink"><X size={14} /></button>
          </div>
          <p className="mt-2 rounded-lg bg-[#f9fcfc] p-2 text-xs italic text-slate-500">"{connectedTerms.text}"</p>
          {connectedTerms.terms.length > 0 ? <div className="mt-2 space-y-2">
            {connectedTerms.terms.map(t => <div key={t.id}><p className="text-xs font-extrabold text-ink">{t.name}</p><p className="text-[11px] leading-relaxed text-slate-500">{t.definition}</p></div>)}
      ...
      <AnimatePresence>
        ...
        >
          ...
          >
            <h3 className="text-base font-extrabold text-ink">New Flashcard</h3>
            ...
            <label className="mt-4 block">
              ...
              <textarea value={cardEditor.front} onChange={e => setCardEditor(c => c && { ...c, front: e.target.value })} rows={2} className="w-full resize-none rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-teal-400" />
            </label>
            <label className="mt-3 block">
              <span className="mb-1 block text-[11px] font-bold uppercase tracking-wide text-slate-500">Back</span>
              <textarea value={cardEditor.back} onChange={e => setCardEditor(c => c && { ...c, back: e.target.value })} rows={2} placeholder="Write the answer..." className="w-full resize-none rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-teal-400" />
            </label>
            <label className="mt-3 block">
              <span className="mb-1 block text-[11px] font-bold uppercase tracking-wide text-slate-500">Add to deck (optional)</span>
              <select value={cardEditorDeckId} onChange={e => setCardEditorDeckId(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-ink outline-none focus:border-teal-400">
                <option value="">Just save to My Cards</option>
                {decks.map(d => <option key={d.id} value={d.id}>{d.name} ({d.cardIds.length})</option>)}
                <option value="new">+ New deck…</option>
              </select>
              {cardEditorDeckId === "new" && <input autoFocus value={cardEditorNewDeckName} onChange={e => setCardEditorNewDeckName(e.target.value)} placeholder="Deck name" className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold text-ink outline-none focus:border-teal-400" />}
            </label>
            <div className="mt-4 flex gap-2">
              <button type="button" onClick={() => setCardEditor(null)} className="flex-1 cursor-pointer rounded-full border border-slate-200 px-4 py-2 text-sm font-bold text-slate-500">Cancel</button>
    ...
    <AnimatePresence initial={false}>
      {panelOpen && <motion.aside
        ...
        className="sticky top-0 hidden h-screen min-w-[360px] max-w-[460px] shrink-0 overflow-hidden border-l border-slate-200 bg-white shadow-soft lg:block"
      >
