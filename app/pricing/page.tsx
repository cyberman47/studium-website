...
import { Billing, comparison, formatPrice, plans } from "@/lib/plans";
      ...
      <div className="container-page">
        ...
        <Reveal delay={.05} className="mt-8 flex justify-center">
          <div className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white p-1 shadow-soft">
            <button type="button" onClick={() => setBilling("monthly")} className={`cursor-pointer rounded-full px-4 py-2 text-xs font-extrabold transition ${billing === "monthly" ? "bg-ink text-white" : "text-slate-500 hover:text-heading"}`}>Monthly</button>
            <button type="button" onClick={() => setBilling("yearly")} className={`flex cursor-pointer items-center gap-1.5 rounded-full px-4 py-2 text-xs font-extrabold transition ${billing === "yearly" ? "bg-ink text-white" : "text-slate-500 hover:text-heading"}`}>Yearly<span className="rounded-full bg-teal-100 px-2 py-0.5 text-[10px] font-extrabold text-teal-700">Save up to 36%</span></button>
          </div>
        </Reveal>
          ...
          <table className="w-full min-w-[640px] border-separate border-spacing-0 overflow-hidden rounded-3xl border border-slate-100 shadow-soft">
            ...
            <tbody>
              {comparison.map(([feature, ...values], i) => <tr key={feature} className={i % 2 ? "bg-white" : "bg-[#fbfdfd]"}>
                <td className="border-t border-slate-100 px-5 py-4 text-sm font-bold text-heading">{feature}</td>
